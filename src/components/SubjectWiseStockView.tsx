...
import { 
  Bookmark, Search, Printer, Download, Eye, BookOpen, AlertCircle, TrendingUp, BarChart4, Sparkles 
} from "lucide-react";
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/50 pb-5 no-print">
        <div>
          <h1 className="text-xl sm:text-2xl font-display font-extrabold text-slate-900 flex items-center gap-2">
            ...
            <span>Subject-wise Stock Explorer</span>
          </h1>
          <p className="text-slate-500 text-xs sm:text-sm mt-1 font-medium">
            Analyze syllabus titles, copies, and financial values Segmented by academic subjects.
          </p>
        </div>
      ...
      <div className="glass-panel border border-white/60 rounded-2xl p-5 space-y-4 no-print shadow-sm">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
          ...
          <div>
            <label className="block text-slate-500 font-bold mb-1.5">Syllabus Subject Dimension</label>
            <select
          ...
          <div>
            <label className="block text-slate-500 font-bold mb-1.5">Target Grade Class</label>
            <select
              value={selectedClass}
          ...
          <div>
            <label className="block text-slate-500 font-bold mb-1.5">Live Keyword Filter</label>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="glass-card border border-slate-200/50 rounded-2xl p-4 shadow-sm">
          ...
          <div className="text-[9px] text-slate-400 mt-1 font-medium">Titles matching filters</div>
        </div>
        ...
        <div className="glass-card border border-slate-200/50 rounded-2xl p-4 shadow-sm col-span-2 sm:col-span-1">
          <p className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold block">Asset Cost</p>
          <p className="text-sm font-bold text-slate-800 mt-2 truncate">PKR {summaryMetrics.totalCostVal.toLocaleString()}</p>
          <div className="text-[9px] text-slate-400 mt-1 font-medium">Purchase cost valuation</div>
        </div>
        ...
        <div className="glass-card border border-slate-200/50 rounded-2xl p-4 shadow-sm col-span-2 sm:col-span-1">
          ...
          <div className="text-[9px] text-slate-400 mt-1 font-medium">Selling revenue potential</div>
        </div>
      ...
      <div className="glass-panel rounded-2xl border border-white/60 overflow-hidden shadow-sm">
        <div className="bg-white/80 px-5 py-4 border-b border-slate-100 flex items-center justify-between text-xs font-bold text-slate-800">
          <span className="font-display font-bold text-slate-800 flex items-center gap-1.5">
            ...
            <span>Subject-wise Inventory Ledger</span>
          </span>
          <span className="text-slate-400 font-medium font-mono text-[10px] bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-lg">Total records: {filteredBookStock.length}</span>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-50/50 text-slate-500 uppercase text-[9px] font-mono border-b border-slate-100 font-bold">
              <tr>
                ...
                <th className="px-5 py-3">Subject Segment</th>
                <th className="px-5 py-3">Class Segment</th>
              ...
              {filteredBookStock.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-12 text-slate-400 font-mono font-medium">
                    No books match the selected search and subject filters.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.book.id} className="hover:bg-white/40 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5 font-medium">{item.publisherName} {item.book.ISBN ? `| ISBN: ${item.book.ISBN}` : ""}</p>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-slate-500 font-medium">{item.subjectName}</td>
                    <td className="px-5 py-4 text-slate-600 font-bold">{item.className}</td>
